...
/*======================================================================================
函数定义	：				开始跑场
函数参数	：				方案：暂定1为逆时针(右侧激光触发)，2为顺时针(左侧激光触发)
函数返回值：				无
=======================================================================================*/
void GoGoGo(int plan)
{
	...
	switch(state)
	{
		case 1:
		{
			if(FirstRound(plan,FIRST_SPEED) == 1)
				state = 2;
		}break;
		case 2:
		{
...
(WIDTH为小车宽度)
=======================================================================================*/
int FirstRound(int plan,int speed)
{
	int ret = 0;	//返回值
	static int state = 1;
	...
	if(plan == 1)	//逆时针
	{
		switch(state)
		{
			...
			case 3:
			{
				...
				if(Position_t.Y <= 1700 - WIDTH/2 + FIR_ADV)
					state = 4;
			}break;
			case 4:
			{
