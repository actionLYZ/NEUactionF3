...
void AdcInit()
{
  ...
  //先初始化ADC1通道4 IO口
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4|GPIO_Pin_5;//PA4 通道4
...
u16 Get_Adc(u8 ch)   
{
	...
	if(ch == 14)//右侧激光
		dis = ADC_GetConversionValue(ADC1) * 0.9451 + 409.08;
	else if(ch == 15)
		dis = ADC_GetConversionValue(ADC1) * 0.9445 + 405.6;
	return dis;	//返回最近一次ADC1规则组的转换结果
}

u16 Get_Adc_Average(u8 ch,int times)
{
	u32 temp_val=0;
	int t;
	for(t=0;t<times;t++)
