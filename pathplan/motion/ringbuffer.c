...
int InputPoints2RingBuffer(Pose_t *points,int num)
{
	...
	for(int i = 1;i < n;i++)
	{
		m1 = GetRingBufferPoint(i).x;
		m2 = GetRingBufferPoint(i + 1).y;
		m3 = GetRingBufferPointAngle(i);
		m4 = GetRingBufferPointAngle(i+1);
		//求出本段曲线的曲线长度
		tempLenth = CaculateBsplineLen(GetRingBufferPoint(i),GetRingBufferPoint(i + 1),GetRingBufferPointAngle(i),GetRingBufferPointAngle(i+1));
		//设置该点距离起点路径长度
